...
function getSurvivorNames() {
    const configs = dbdStorage.getSurvivorConfigs();
    const names = [];
...
function getRulesByMode(mode) {
    const generalRules = [
        ...
        "Match ignored if the killer is AFK or disconnects early."
    ];
    ...
    const extraRule = {
        ...
        squad: "Match ignored if the killer disconnects early."
    };
...
function createTableRow(match, index) {
...
function renderTable(matches) {
    ...
    tableBody.innerHTML = matches
        .map((match, index) => createTableRow(match, index))
        .join("");
}
